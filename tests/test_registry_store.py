...
@pytest.mark.parametrize(
    "version", [REGISTRY_SCHEMA_VERSION - 1, REGISTRY_SCHEMA_VERSION + 1]
)
def test_incompatible_registry_is_rejected_without_writes(tmp_path: Path, version: int) -> None:
    ...
    with pytest.raises(IntegrityError, match="incompatible CueFlow registry schema"):
        Registry(database)
...
def test_artifact_file_precedes_pointer_and_pointer_rolls_back(tmp_path: Path) -> None:
    ...
    nonempty_database = tmp_path / "nonempty.sqlite3"
    with sqlite3.connect(nonempty_database) as connection:
        connection.execute("CREATE TABLE projects (project_id TEXT PRIMARY KEY)")
    with pytest.raises(IntegrityError, match="incompatible CueFlow registry schema"):
        Registry(nonempty_database)
    with sqlite3.connect(nonempty_database) as connection:
...
def test_interrupted_run_can_only_reopen_for_targeted_retry(tmp_path: Path) -> None:
    project = initialize_project(tmp_path / "project", "Test")
    try:
        run_id = project.registry.create_source_run(
            project.project_id,
        ...
        assert project.registry.recover_running_source_runs() == [run_id]
