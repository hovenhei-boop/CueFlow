# CueFlow v0.1 Architecture

状态：冻结基线

## 1. 产品边界

CueFlow v0.1 处理已经剪辑完成的视频或音频，唯一主链为：
...
目标是忠实转写、精确时间轴、纠错和导出。v0.1 不主动编辑或净化成片内容，不删除实际说出的 Atom，不做视频编辑、视觉理解或额外用户界面。

## 2. 核心不变量

1. SourceAsset 身份由原文件完整 SHA-256 和字节长度确定；项目只保存外部绝对路径，不复制、移动或删除源媒体。
...
`project` 完整读取用户路径以登记 SHA-256、长度、格式和媒体类型。重新执行 Media Prep 前必须重新校验路径与内容身份。路径缺失时抛 `SourceMissingError`；不猜测新路径，也不提供 relink。
...
一个新 Run 中，每 Chunk 初始最多 4 个 Semantic Attempt；该上限属于版本化 QA Ruleset 的运行规则。每个成功 Attempt 创建 Transcript Artifact、Invocation 和 exact input bindings。Glossary 稳定闭环只使用冻结规则：
...
连续两次候选序列相同且与 term 一致时 resolved；连续两次相同但仍冲突时接受实际转写并产生非阻塞 `stable_glossary_conflict`；4 次内无法连续稳定时产生非阻塞 `unstable_glossary_conflict`。

只有显式 targeted retry 可以为目标 Chunk 重置 4-Attempt budget；同一原 Run、同一 Chunk 固定最多重置 2 次，因此绝对上限为 12 次。两次 reset 是 v0.1 SQLite window 数据模型约束，不是运行时可配置项。进入 `sending` 的 Attempt 计入预算；`definitely_not_sent` 不计。reset count 持久化并可审计。
...
Artifact publish 顺序为：写临时文件、flush/fsync、原子重命名、SQLite 事务登记 Artifact/dependency、切换 current/stale pointer、commit。
...
v0.1 唯一界面是现有 CLI：`init`、`glossary set`、`asset add`、`run`、`status`、`retry`。不增加模型、Provider、Chunk 或字幕风格选择。
