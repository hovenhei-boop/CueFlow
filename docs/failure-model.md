...
用户可见 SRT 只能由已登记的内部 `srt_render` Artifact 生成；它的 Subtitle 与 QA 依赖链必须 current 且非 stale。
...
Warning（包括 Chunk 边界疑似重复、最终未解决的 Review Issue 和 `timeline_status = unverified`）不阻塞导出。未来 GUI 可以展示这些内部 warning，但 v0.1 不生成公开 Review Report，也不要求 `human_verified_final` 状态。
...
- `CLOUD_PROFILE`：语义转写会把当前 Chunk 音频、Effective Glossary 词条及必要的语言/转写配置上传到 Qwen3.5-Omni；返回 Transcript 后，原 Chunk 与文字交给本地 Qwen3-ForcedAligner-0.6B。
...
低 confidence 或可疑文字先形成内部 Issue。只有白名单内的 QA rule code 可以触发自动返工，且只有 Orchestrator 可以请求 Semantic Transcriber 重新处理对应 Chunk；QA 不得直接改字。每个 Chunk、每次 Run 最多 4 次 Semantic Attempt（首次 + 最多 3 次自动返工）。每次返工创建新的 Transcript、Alignment、Invocation 和依赖记录，并只切换该 Chunk 的 Current Pointer。

返工成功且新 QA 不再发现问题时，Issue 标记为 `resolved`，不展示给用户。达到 4 次 Semantic Attempt 上限或证据不足时立即停止返工，标记为 `unresolved` 并生成用户界面 ReviewIssue。返工循环不能无限运行，也不能把一次 `delivery_ambiguous` 云请求当作普通自动返工再次发送。
...
- QA 返工只能由白名单 rule code 触发、生成新版本，并遵守每个 Chunk/Run 最多 4 次 Semantic Attempt 的上限；
