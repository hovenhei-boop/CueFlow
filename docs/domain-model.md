...
v0.5.1 只构建和管理这些数据，不把 Project Lexicon 或 Official Pack 自动注入 Effective Glossary/Source。未来消费必须先做相关性与容量限制，并把所选 entry、Pack version 和选择算法一并冻结为 Source Run 的不可变 Effective Glossary Snapshot；targeted retry 继续绑定原 snapshot。
